...
if ($_REQUEST['act'] == 'list')
{
    ...
    $account['voucher_amount'] = get_total_amount($start_date, $end_date);//充值總額
    $account['to_cash_amount'] = get_total_amount($start_date, $end_date, 1);//提現總額
    ...
    $account['user_money']     = price_format($money_list['user_money']);   //用戶可用餘額
    $account['frozen_money']   = price_format($money_list['frozen_money']);   //用戶凍結金額
    ...
    $account['surplus']        = price_format($money_list['surplus']);   //交易使用餘額
    $account['integral_money'] = price_format($money_list['integral_money']);   //積分使用餘額
