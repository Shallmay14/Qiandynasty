...
if ($_REQUEST['act'] == 'install')
{
    ...
    /* 增加ucenter設置時先檢測uc_client與uc_client/data是否可寫 */
...
if ($_REQUEST['act'] == 'check_user')
{
    ...
    if ($post_user_list)
    {
        ...
        if ($method == 2 )
        {
            ...
            /* 檢查改名後用戶是否和論壇用戶有重名 */
            $error_user_list = $cls_user->test_conflict($test_user_list);   //檢查
            if ($error_user_list)
            {
...
{
    /* 先處理要改名的用戶，改名用戶要先檢查是否有重名情況，有則標記出來 */
    $alias = array();
...
if ($_REQUEST['act'] == 'points_set')
{
    ...
    $user = &init_users();
    $points = $user->get_points_name(); //獲取商城可用積分
