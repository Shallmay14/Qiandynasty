...
if ($_REQUEST['act'] == 'setting')
{
    ...
    {
        //clear_tpl_files(false, '.dwt.php'); // 清除對應的編譯文件
        clear_cache_files();
...
if ($_REQUEST['act'] == 'restore_library')
{
    ...
    $lib_backup = str_replace("0xa", '', $lib_backup); // 過濾 0xa 非法字符
 ...
 *
 * @access  public
 * @param   string  $curr_template  模版名稱
 * @param   string  $lib_name       庫項目名稱
 * @return  array
 */
function load_library($curr_template, $lib_name)
{
    $lib_name = str_replace("0xa", '', $lib_name); // 過濾 0xa 非法字符
 ...
 *
 * @access  public
 * @param   string  $tpl_name       模版名稱
 * @param   int     $flag           1，AJAX數據；2，Array
 * @return
 */
function read_tpl_style($tpl_name, $flag=1)
{
    ...
    while ($file = readdir($tpl_style_dir))
    {
        ...
        {
            if (preg_match("/^(style|style_)(.*)*/i", $file)) // 取模板風格縮略圖
            {
                $start = strpos($file, '.');
