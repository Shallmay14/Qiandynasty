...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    $data = $modules[0];
    /* 對支付費用判斷。如果data['pay_fee']為false無支付費用，為空則說明以配送有關，其它可以修改 */
...
elseif (isset($_POST['Submit']))
{
    ...
    else
    {
        ...
        if ($db->GetOne($sql) > 0)
        {
            /* 該支付方式已經安裝過, 將該支付方式的狀態設置為 enable */
            $sql = "UPDATE " . $ecs->table('payment') .
                   "SET pay_name = '$_POST[pay_name]'," .
                   "    pay_desc = '$_POST[pay_desc]'," .
                   "    pay_config = '$pay_config'," .
...
/*------------------------------------------------------ */
//-- 卸載支付方式 ?act=uninstall&code={$code}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'edit_pay_fee')
{
    ...
    {
        $pay_fee = make_semiangle($pay_fee); //全角轉半角
