...
if ($_REQUEST['act'] == 'backup')
{
    ...
    $allow_max_size = return_bytes(@ini_get('upload_max_filesize')); // 單位為字節
    $allow_max_size = $allow_max_size / 1024; // 轉換單位為 KB
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    {
        $sql_file_name = str_replace("0xa", '', trim($_REQUEST['sql_file_name'])); // 過濾 0xa 非法字符
    ...
    /* 變量驗證 */
    $allow_max_size = intval(@ini_get('upload_max_filesize')); //單位M
    if ($allow_max_size > 0 && $max_size > ($allow_max_size * 1024))
    {
        $max_size = $allow_max_size * 1024; //單位K
    }
...
function num_bitunit($num)
{
    ...
    {
       if ($num >= pow(2, 10 * $key) - 1) // 1024B 會顯示為 1KB
       {
