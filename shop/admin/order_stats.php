...
if ($_REQUEST['act'] == 'list')
{
    ...
    {
        $today      = strtotime(local_date('Y-m-d'));   //本地時間
        $start_date = $today - 86400 * 6;
        $end_date   = $today + 86400;               //至明天零時
    }
    ...
    $smarty->assign('click_count',         $click_count);         //商品總點擊數
    $smarty->assign('click_ordernum',      $click_ordernum);      //每千點訂單數
    $smarty->assign('click_turnover',      price_format($click_turnover));  //每千點購物額
...
/*------------------------------------------------------ */
 /**
  * 取得訂單概況數據(包括訂單的幾種狀態)
  * @param       $start_date    開始查詢的日期
  * @param       $end_date      查詢的結束日期
  * @return      $order_info    訂單概況數據
  */
