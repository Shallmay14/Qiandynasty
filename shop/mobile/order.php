...
$flow_type = 0;
    $_LANG['gram'] = '克';
    $_LANG['kilogram'] = '千克';
    $tips = '訂單已經生成，如果您已經登陸，可以在用戶中心查看訂單詳情!';
...
elseif($_REQUEST['act'] = 'done')
{
    ...
    {
        log_account_change($order['user_id'], $order['surplus'] * (-1), 0, 0, 0, sprintf('支付訂單 %s', $order['order_sn']));
    }
    if ($order['user_id'] > 0 && $order['integral'] > 0)
    {
        log_account_change($order['user_id'], 0, 0, 0, $order['integral'] * (-1), sprintf('支付訂單 %s', $order['order_sn']));
    }
    /* 如果使用庫存，且下訂單時減庫存，則減少庫存 */
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
    {
    ...
    $smarty->assign('total',      $total);
    $smarty->assign('goods_list', $cart_goods);
    $smarty->assign('order_submit_back', sprintf('您可以 %s 或去 %s', '<a href="index.php">返回首頁</a>', '<a href="user.php">用戶中心</a>')); // 返回提示
