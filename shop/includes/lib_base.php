 ...
 * @param: $email[string]       接收人郵件地址
 * @param: $subject[string]     郵件標題
 * @param: $content[string]     郵件內容
 * @param: $type[int]           0 普通郵件， 1 HTML郵件
 * @param: $notification[bool]  true 要求回執， false 不用回執
 *
 * @return boolean
 */
...
/**
 * 將對象成員變量或者數組的特殊字符進行轉義
 *
 * @access   public
 * @param    mix        $obj      對象或者數組
 * @author   Xuan Yan
 *
 * @return   mix                  對象或者數組
 */
...
function trim_right($str)
{
    ...
    /* 有非獨立的字符，先把非獨立字符去掉，再驗證非獨立的字符是不是一個完整的字，不是連原來前導字符也截取掉 */
